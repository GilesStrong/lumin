...
def fold_lr_find(fold_yielder:FoldYielder, model_builder:ModelBuilder, bs:int,
                 train_on_weights:bool=True, shuffle_fold:bool=True, n_folds:int=-1, lr_bounds:Tuple[float,float]=[1e-5, 10]) -> List[LRFinder]:
    ...
    if n_folds != 1:
        plot_lr_finders(lr_finders, loss='loss', cut=-2)
    else:
