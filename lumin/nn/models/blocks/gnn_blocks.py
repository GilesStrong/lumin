...
class GraphCollapser(AbsGraphBlock):
    ...
    def forward(self, x:Tensor) -> Tensor:
        ...
        if self.global_feat_vec and self.f_initial_outs is not None and self.f_final_outs is None:
            x = torch.cat([x,x.mean(1, keepdim=True).repeat_interleave(repeats=x.shape[1],dim=2).transpose(1,2)],dim=2) 
        x = self.f_inital(x)
        if self.n_sa_layers > 0:
            outs = [x]
...
class GravNetLayer(AbsGraphBlock):
    ...
    def forward(self, x:Tensor) -> Tensor:
        ...
        # Concat means
        if self.cat_means: x = torch.cat([x,x.mean(1, keepdim=True).repeat_interleave(repeats=x.shape[1],dim=-1).transpose(1,2)],dim=2)
