...
class GNNHead(AbsMatrixHead):
    ...
    def forward(self, x:Union[Tensor,Tuple[Tensor,Tensor]]) -> Tensor:
        ...
        if self.cat_means:
            if self.row_wise: x = torch.cat([x,x.mean(1, keepdim=True).repeat_interleave(repeats=x.shape[1],dim=2).transpose(1,2)],dim=2)
            else:             x = torch.cat([x,x.mean(2, keepdim=True).repeat_interleave(repeats=x.shape[2],dim=1).transpose(1,2)],dim=1)
