...
class AbsHead(AbsBlock):
    def __init__(self, n_cont_in:int, cat_embedder:Optional[CatEmbedder]=None, 
        ...
        self.n_cont_in,self.cat_embedder = n_cont_in,cat_embedder
...
class CatEmbHead(AbsHead):
    ...
    Examples::
        ...
        >>> head = CatEmbHead(n_cont_in=25, cat_embedder=CatEmbedder.from_fy(train_fy))
        >>> head = CatEmbHead(n_cont_in=25, cat_embedder=CatEmbedder.from_fy(train_fy), do_cont=0.1, do_cat=0.05)
        >>> head = CatEmbHead(n_cont_in=25, cat_embedder=CatEmbedder.from_fy(train_fy), lookup_init=lookup_uniform_init)
    '''

    def __init__(self, n_cont_in:int, do_cont:float=0, do_cat:float=0, cat_embedder:Optional[CatEmbedder]=None, 
                 lookup_init:Callable[[str,Optional[int],Optional[int]],Callable[[Tensor],None]]=lookup_normal_init, freeze:bool=False):
        super().__init__(n_cont_in=n_cont_in, cat_embedder=cat_embedder, lookup_init=lookup_init, freeze=freeze)
