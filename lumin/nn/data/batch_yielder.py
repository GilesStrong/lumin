...
class BatchYielder:
    def __init__(self, inputs:np.ndarray, targets:np.ndarray, bs:int, objective:str, weights:Optional[np.ndarray]=None, shuffle=True, use_weights:bool=True, bulk_move=True):
        self.inputs,self.targets,self.weights,self.bs,self.objective,self.shuffle,self.use_weights,self.bulk_move = inputs,targets,weights,bs,objective,shuffle,use_weights,bulk_move
    ...
    def __iter__(self) -> List[Tensor]:
        ...
        if self.bulk_move:
            ...
            for i in range(0, len(inputs)-self.bs+1, self.bs):
                if weights is None:
                    yield inputs[i:i+self.bs], targets[i:i+self.bs], None
                else:
                    yield inputs[i:i+self.bs], targets[i:i+self.bs], weights[i:i+self.bs]                    
