...
def fold2foldfile(df:Optional[pd.DataFrame], out_file:h5py.File, fold_idx:int,
...
def df2foldfile(df:Optional[pd.DataFrame], n_folds:int, cont_feats:List[str], cat_feats:List[str],
    ...
    add_meta_data(out_file=out_file, feats=df.columns if df is not None else [], cont_feats=cont_feats, cat_feats=cat_feats, cat_maps=cat_maps,
                  ...
                  tensor_name=tensor_name, tensor_shp=tensor_data[0].shape if tensor_data is not None else None, tensor_is_sparse=tensor_as_sparse,
                  target_tensor_shp=tensor_target[0].shape if tensor_target is not None else None, tensor_target_is_sparse=tensor_target_as_sparse)
...
def add_meta_data(out_file:h5py.File, feats:List[str], cont_feats:List[str], cat_feats:List[str], cat_maps:Optional[Dict[str,Dict[int,Any]]],
                  ...
                  matrix_vecs:Optional[List[str]]=None, matrix_feats_per_vec:Optional[List[str]]=None, matrix_row_wise:Optional[bool]=None,
                  tensor_name:Optional[str]=None, tensor_shp:Optional[Tuple[int]]=None, tensor_is_sparse:bool=False, 
                  target_tensor_shp:Optional[Tuple[int]]=None, tensor_target_is_sparse:bool=False) -> None:
    r'''
    ...
    if target_tensor_shp is not None:
        grp.create_dataset('target_tensor', data=json.dumps({'shape': target_tensor_shp, 'is_sparse':tensor_target_is_sparse}))
